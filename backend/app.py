...
def admin_dashboard():
    """Protected admin dashboard — shows all Users from DynamoDB."""
    ...
    try:
        ...
        # Convert Decimal types to int/float for template rendering
        for user in users:
            if 'mobile' in user:
...
def admin_update():
    """Update a user record in DynamoDB (first_name, last_name, email)."""
    if not session.get('sf_access_token'):
...
def admin_delete():
    """Delete a user record from DynamoDB."""
    if not session.get('sf_access_token'):
        return jsonify({"error": "Unauthorized"}), 401
